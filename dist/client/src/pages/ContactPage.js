...
const ContactsList_1 = __importDefault(require("../components/ContactsList"));
const Search_1 = __importDefault(require("../components/Search"));
const app_api_1 = require("../store/app/app.api");
function ContactPage() {
    const { data: contacts, error, isLoading } = (0, app_api_1.useGetAllContactsQuery)('');
    if (!isLoading && (contacts === null || contacts === void 0 ? void 0 : contacts.length) === 0) {
        return <div className='text-lg font-semibold'>There are no contacts!</div>;
    }
    if (error) {
    ...
    return (<div className='grid grid-cols-1 gaps-4 mx-2'>
      <Search_1.default />
      <ContactsList_1.default contacts={contacts}/>
    </div>);
}
